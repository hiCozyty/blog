...
4. [UDP Security](#4-udp-security)
    ...
    4.4 [SessionID is cleartext. Why it doesn't help an attacker](#44-sessionid-is-cleartext-why-it-doesnt-help-an-attacker)
...
1. Implementation Gap: Most of what I read during my studies explained what to protect and why — but glossed over the messy details of actually implementing it. I wanted to fill that gap for myself by building something where I had to justify every decision.
...
So I set out to create a voice-native assistant that lives on the apple watch. I wanted to build something real that I'd actually use, and make sure that it was secure.
...
##### 1.3 Threat Model
These are the threats that I was worried about during development:
...
I did not worry about the threats below:
...
`ca.crt` is bundled with the Xcode app target.
...
* Hardware acceleration: AES-GCM benefits from dedicated crypto acceleration on Apple S-series hardware compared to software only CBC-HMAC implementation.
...
```
[16B Session ID][2B Sequence][12B Nonce][Ciphertext][16B Auth Tag]
```
...
##### 4.4 SessionID is cleartext. Why it doesn't help an attacker
The session ID in each UDP packet is visible to anyone watching my network traffic.
This might sound alarming, but knowing the session ID alone is insufficient to carry out an injection attack.

For the server to accept a packet, it must pass `AES-GCM` authentication. This means that the packet must carry a valid 16-byte auth tag. This is a message authentication code (MAC) computed using the session key:
```
auth tag = GHASH(session key, ciphertext + nonce)
```
This proves the packet was produced by someone who holds the session key, but both sides share the same key. 

And remember, sessionKey is only created during successful mTLS authentication.
Without the session key, an attacker cannot produce this tag. They'd be randomly guessing a 128-bit value — there are 2¹²⁸ possible combinations, roughly 340 undecillion.

An eaevesdropper might learn that there is an existing session, and when I might be talking and how much data is being sent. The content of the audio data remains protected.
